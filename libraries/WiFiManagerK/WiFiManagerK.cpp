...
void WiFiManager::handleRoot() {
  ...
  page.replace("{v}", "Op&ccedil&otildees");
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  head.replace("{v}", "Configura&ccedil&atildeo");
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      server->sendContent("Nenhuma rede encontrada. Por favor atualize a p&aacutegina para repetir a busca.");
    }
    else {
       server->sendContent(F("<P ALIGN=CENTER><h3>Por favor clique na rede desejada <br>"));
       server->sendContent(F("e digite a senha, caso exista, <br>"));
       server->sendContent(F("no campo [senha].</h3><br>"));
      for (int i = 0; i < n; ++i)
      {
...
void WiFiManager::handleWifiSave() {
  ...
  String head = FPSTR(HTTP_HEAD);
  head.replace("{v}", "Credenciais Salvas");
  server->sendContent(head);
  server->sendContent_P(HTTP_SCRIPT);
  server->sendContent_P(HTTP_STYLE);
  server->sendContent_P(HTTP_HEAD_END);
...
void WiFiManager::handleInfo() {
  ...
  page += F("</dd>");
  page += F("<br>");
  page += F("<dt>Tamanho da Mem&oacuteria Flash</dt><dd>");
  page += ESP.getFlashChipSize();
  page += F(" bytes</dd>");
  page += F("<br>");
  page += F("<dt>Endere&ccedilo de IP (Modo AP)</dt><dd>");
  page += WiFi.softAPIP().toString();
  page += F("</dd>");
  page += F("<br>");
  page += F("<dt>Endere&ccedilo MAC (Modo AP)</dt><dd>");
  page += WiFi.softAPmacAddress();
  page += F("</dd>");
  page += F("<br>");
  page += F("<dt>Endere&ccedilo MAC (Modo Estruturado)</dt><dd>");
